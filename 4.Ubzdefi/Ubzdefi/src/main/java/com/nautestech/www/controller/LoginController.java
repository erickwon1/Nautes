...
public class LoginController {
	...
	public ResponseEntity<?> preLogin(HttpServletRequest request, @RequestParam("login_id") String login_id,
		...
		try {
			...
			inputLog.inputLog(String.format("로그인 실패 (존재하지 않는 사용자) -  IP: %s", request.getRemoteAddr()));
		...
		if (!encoder.matches(login_pwd, user.getPassword())) {
			inputLog.inputLog(String.format("로그인 실패 - 사용자: %s, IP: %s", login_id, request.getRemoteAddr()));
		...
		if (admin.getAuth_secret() == null || admin.getAuth_secret().isEmpty()) {
			return ResponseEntity.ok("NOT OTP INFORM");
		}
	...
	public ResponseEntity<?> changePassword(HttpServletRequest request, @RequestParam("login_id") String login_id) {
		...
		Admin admin = adminService.findById(login_id);
		if (admin == null) {
			inputLog.inputLog(String.format("비밀번호 변경 실패 (존재하지 않는 사용자) -  IP: %s", request.getRemoteAddr()));
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).header("Content-Type", "text/plain; charset=UTF-8")
					.body("존재하지 않는 아이디입니다.");
		}
	...
	public ResponseEntity<?> verifyOTP(HttpServletRequest request, @RequestParam String login_id,
		...
		if (session == null) {
			inputLog.inputLog(String.format("세션 만료 - 사용자: %s, IP: %s", login_id, request.getRemoteAddr()));
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).header("Content-Type", "text/plain; charset=UTF-8")
					.body("세션이 만료되었습니다.");
		}
		...
		if (!googleOTPService.verifyCode(savedSecret, Integer.parseInt(code))) {
			inputLog.inputLog(String.format("OTP 인증 실패 - 사용자: %s, IP: %s", login_id, request.getRemoteAddr()));
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).header("Content-Type", "text/plain; charset=UTF-8")
					.body("OTP 인증번호가 틀렸거나 만료되었습니다.");
		}
